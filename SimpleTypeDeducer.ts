...
export class SimpleTypeDeducer extends LowerBoundTypeDeducer {
    getTypeFor(calls: FunctionCalls): FunctionTypeDefinition {
        ...
        for (let i = 0; i < definition.argTypes.length; i++) {
            definition.argTypes[i] = definition.argTypes[i].roundUp(new ArgValidator(calls.info, calls.calls, i), this.parameters);
        }

        definition.returnValueType = definition.returnValueType.roundUp(new NoopValidator(), this.parameters);
        return definition;
    }
}